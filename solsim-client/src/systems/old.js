...
const convertOldDataToOrbitBodies = (oldData) => {
    return oldData.map(item => ({
      name: item.name,
      mass: item.mass,
      radius: item.diameter / 2, // Assuming radius is half the diameter
      pixelRadius: item.pixelSize,
      luminosity: 0, // Assuming luminosity is not provided in the old data
      color: item.color,
      position: {
        x: item.x,
        y: item.y,
